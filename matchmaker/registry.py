...
``STREAM_BUILDERS``
    ``stream: <name>`` on a method: how the input stream is built, for the rare
    method that cannot use the standard one. Registered with
    :func:`stream_builder`.
...
@provider("ensemble_members")
def _ensemble_members(mm):
    """The ensemble's member followers, built once and shared with its stream."""
    from matchmaker.ensemble.build import ensemble_members

    return ensemble_members(mm)
...
# ---------------------------------------------------------------------------
# Stream builders: how the input stream is built
# ---------------------------------------------------------------------------
STREAM_BUILDERS: Dict[str, Callable[[Any, bool], Any]] = {}


def stream_builder(name: str) -> Callable:
    """Register a ``stream: <name>`` strategy.

    The builder is ``build(mm, wait) -> stream``. A method declares one only
    when the standard single-modality AudioStream/MidiStream cannot serve it;
    everything else omits ``stream`` and Matchmaker builds the usual one.
    """

    def decorate(func):
        STREAM_BUILDERS[name] = func
        return func

    return decorate


@stream_builder("merged")
def _stream_merged(mm, wait):
    """One raw stream per modality present, merged into a tagged queue.

    For the ensemble, whose members may read different modalities than the
    ensemble's own ``input_type``.
    """
    from matchmaker.ensemble.build import build_merged_stream

    return build_merged_stream(mm, wait)
...
class MethodSpec:
    ...
    def parse(cls, name: str, input_type: str, raw: dict) -> "MethodSpec":
        ...
        if stream is not None and stream not in STREAM_BUILDERS:
            raise ValueError(
                f"{where}: unknown stream builder '{stream}'. "
                f"Known: {sorted(STREAM_BUILDERS)}"
            )
    ...
    def build_stream(self, mm, wait):
        """The method's own stream, or None to use Matchmaker's standard one."""
        if self.stream is None:
            return None
        return STREAM_BUILDERS[self.stream](mm, wait)
...
class Registry:
    ...
    def build_stream(self, mm, method: str, wait: bool):
        """The method's own stream, or None if it uses the standard one."""
        return self.method(mm.input_type, method).build_stream(mm, wait)
