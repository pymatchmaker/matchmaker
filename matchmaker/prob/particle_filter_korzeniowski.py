...
class ParticleFilterKorzeniowski(OnlineAlignment):
    ...
    def set_position(self, beat: float, strength: float = 1.0) -> bool:
        ...
        relocated = int(mask.sum())
        if relocated > 0:
            # Re-seed the relocated particles at the current tempo belief rather
            # than the notated one: by the time feedback arrives the performance
            # may be far from the tempo the filter started at.
            logtempo = float(np.mean(self.m))
            local_logtempo = float(np.mean(self.l))
            self.x[mask] = beat + self.rng.normal(0, 0.1, relocated)
            self.m[mask] = logtempo
            self.l[mask] = local_logtempo
